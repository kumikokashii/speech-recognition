...
class SpectrogramConcatCLSTM2(UsefulTFGraph):
    ...
    def prebuild(self, cnfg):
        ...
        cnfg.lstm_n_per_time = cnfg.X_img_h + cnfg.n_per_time_after_reducing_conv
    ...
    def build(self, cnfg):
        ...
        with self.as_default():
            ...
            with tf.variable_scope('conv2ds'):                
                conv2ds_output = self.apply_dr_ds_conv(conv1_output, *cnfg.conv2ds_params, end_with_relu=True)                   
                
            with tf.variable_scope('dim_reduce_conv'):
                reduce_conv = self.reduce_conv_dim_for_lstm(conv2ds_output, n_time=cnfg.X_img_w, 
                                                            n_per_time=cnfg.n_per_time_after_conv, 
                                                            n_after_per_time=cnfg.n_per_time_after_reducing_conv)

            with tf.variable_scope('concat'):
                concat = tf.concat([tf.transpose(self.X, [0, 2, 1]), reduce_conv], 2)         
            
            with tf.variable_scope('lstm'):
                lstm_outputs = self.get_lstm_outputs(concat, cnfg.lstm_n_per_time, cnfg.lstm_state_sizes, cnfg.lstm_keep_prob)

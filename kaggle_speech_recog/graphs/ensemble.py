...
class Ensemble(UsefulTFGraph):
    ...
    def build(self, cnfg):
        ...
        with self.as_default():
            ...
            self.X = tf.placeholder(tf.float32, [None, cnfg.X_n_features])
            self.Y = tf.placeholder(tf.float32, [None, cnfg.Y_vector_len])   

            self.logits = self.apply_flat_layers(self.X, cnfg.X_n_features, cnfg.flat_hiddens, cnfg.Y_vector_len)

...
class SpectrogramMaxPoolLSTM(UsefulTFGraph):
    ...
    def get_lstm_outputs(self, inputs, dim_per_time, state_sizes):
        ...
        X = self.apply_batch_normalize(tf.transpose(inputs, [0, 2, 1]))
        ...
        outputs, final_state = tf.nn.dynamic_rnn(multi_cell, X, initial_state=tuple(initial_state_list))        
    ...
    def build(self, cnfg):
        ...
        with self.as_default():
            ...
            with tf.variable_scope('lstm'):
                lstm_outputs = self.get_lstm_outputs(self.X, dim_per_time=cnfg.X_img_h, state_sizes=cnfg.lstm_state_sizes)
                X1 = self.flatten_maxpool_output(lstm_outputs, cnfg.n_lstm_maxpool, cnfg.n_flat)

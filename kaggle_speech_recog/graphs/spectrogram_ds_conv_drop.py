...
class SpectrogramDSConvDropout(UsefulTFGraph):
    ...
    def apply_dr_ds_conv(self, inputs, keep_prob, kernel_size, strides, n_filters):
    ...
    def apply_ave_pool(self, inputs, window, strides):
    ...
    def prebuild(self, cnfg):
        ...
        cnfg.conv1_params = (cnfg.conv1_keep_prob, cnfg.conv1_n_filters, cnfg.conv1_kernel_size, 
                             cnfg.conv1_mp_size, cnfg.conv1_mp_strides)
        cnfg.conv2ds_params = (cnfg.conv2ds_keep_prob, cnfg.conv2ds_kernel_size, cnfg.conv2ds_strides, cnfg.conv2ds_n_filters)
        cnfg.conv3ds_params = (cnfg.conv3ds_keep_prob, cnfg.conv3ds_kernel_size, cnfg.conv3ds_strides, cnfg.conv3ds_n_filters)
        cnfg.conv4ds_params = (cnfg.conv4ds_keep_prob, cnfg.conv4ds_kernel_size, cnfg.conv4ds_strides, cnfg.conv4ds_n_filters)
        cnfg.conv5ds_params = (cnfg.conv5ds_keep_prob, cnfg.conv5ds_kernel_size, cnfg.conv5ds_strides, cnfg.conv5ds_n_filters)
        cnfg.conv6ds_params = (cnfg.conv6ds_keep_prob, cnfg.conv6ds_kernel_size, cnfg.conv6ds_strides, cnfg.conv6ds_n_filters)
        
        cnfg.n_flat = cnfg.b4flat_h * cnfg.b4flat_w * cnfg.conv6ds_n_filters
    ...
    def build(self, cnfg):
        ...
        with self.as_default():
            ...
            with tf.variable_scope('conv1'):
                conv1_output = self.apply_bn_dr_conv_mp(tf.reshape(self.X, [-1, cnfg.X_img_h, cnfg.X_img_w, 1]), 
                                                        *cnfg.conv1_params, skip_relu=True)            

            with tf.variable_scope('conv2ds'):
                conv2_output = self.apply_dr_ds_conv(conv1_output, *cnfg.conv2ds_params)

            with tf.variable_scope('conv3ds'):
                conv3_output = self.apply_dr_ds_conv(conv2_output, *cnfg.conv3ds_params)
                
            with tf.variable_scope('conv4ds'):
                conv4_output = self.apply_dr_ds_conv(conv3_output, *cnfg.conv4ds_params)
                
            with tf.variable_scope('conv5ds'):
                conv5_output = self.apply_dr_ds_conv(conv4_output, *cnfg.conv5ds_params)
                
            with tf.variable_scope('conv6ds'):
                conv6_output = self.apply_dr_ds_conv(conv5_output, *cnfg.conv6ds_params)
                
            with tf.variable_scope('ave_pool'):
                ave_pool = self.apply_ave_pool(conv6_output, cnfg.ave_pool_window, cnfg.ave_pool_strides)
                X1 = tf.reshape(ave_pool, [-1, cnfg.n_flat])  # Reshape for fully-connected

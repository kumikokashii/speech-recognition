...
class SpectrogramConcatCLSTM(UsefulTFGraph):
    ...
    def prebuild(self, cnfg):
        ...
        cnfg.conv_params = (cnfg.conv_keep_prob, cnfg.conv_n_filters, cnfg.conv_kernel_size, 
                            cnfg.conv_mp_size, cnfg.conv_mp_strides)
        
        cnfg.lstm_dim_per_time = cnfg.X_img_h + cnfg.dim_per_time_after_conv
    ...
    def build(self, cnfg):
        ...
        with self.as_default():
            ...
            with tf.variable_scope('conv'):
                conv_output = self.apply_bn_dr_conv_mp(tf.reshape(self.X, [-1, cnfg.X_img_h, cnfg.X_img_w, 1]), 
                                                       *cnfg.conv_params)            

            with tf.variable_scope('concat'):
                reshaped_conv_output = self.reshape_for_lstm(conv_output, cnfg.X_img_w, cnfg.dim_per_time_after_conv)
                concat = tf.concat([tf.transpose(self.X, [0, 2, 1]), reshaped_conv_output], 2)                
            
            with tf.variable_scope('lstm'):
                lstm_outputs = self.get_lstm_outputs(concat, cnfg.lstm_dim_per_time, cnfg.lstm_state_sizes)
                X1 = tf.reshape(lstm_outputs[:, -1, :], [-1, cnfg.n_flat])
